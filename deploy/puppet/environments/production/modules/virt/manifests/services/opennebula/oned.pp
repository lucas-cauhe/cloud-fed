...
){ # Container declaration quitar los session de pam pasar a root y oneadmin la clave de /var/lib/one/.ssh/id_rsa.pub como root $env = $facts['env_vars']
    include storage::ceph::vars
	$imageName = "almalinux_oned"
    $deploy_id = split($oned[hostname], '_')[1]
    $db_passwd = lookup('opennebula', Hash, 'deep')[db_config][one_passwd]
		...
		plan => [
			{
				"type" => "container",
				"actions" => [
					"podman cp $scripts_path/mock_service.sh ${oned[hostname]}:/bin/service"
				]
			},
			{
				"type" => "guest_file",
		       ...
		       TIMEOUT = 2500 ]',
							'dest' => inline_epp($db_config, {'passwd' => $db_passwd, 'ipaddress' => lookup('opennebula')[db][$deploy_id]})
						}
					},
					{
						'path' => '/etc/one/oned.conf',
			...
			#
			{
				...
				"actions" => [
					...
					"podman exec ${leader[hostname]} onedb backup -S ${lookup('opennebula')[db][$deploy_id]} -t mysql -u oneadmin -p $db_passwd -d opennebula -f /var/lib/one/nebula_backup.sql",
					...
					"podman exec ${oned[hostname]} onedb restore -f -u oneadmin -p $db_passwd -d opennebula -S ${lookup('opennebula')[db][$deploy_id]} -t mysql /tmp/nebula_backup.sql",
				...
				"actions" => [
					...
					},
					{
						...
						'replace' => {
							...
							'dest' => "MONITOR_ADDRESS = \"${lookup('opennebula')[vip]}\""
						}
					},
					{
						'path' => '/etc/one/oned.conf',
						'append' => inline_epp(
                            lookup('opennebula', Hash, 'deep')[raft_leader_hooks],
                            {'virtual_ip' => lookup('opennebula')[vip],
                            'id' => $fed_id}
                        )
					}
				]
