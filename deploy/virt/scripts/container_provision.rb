...
params["plan"].each {|action_type| action_type["actions"].each do |action|
  ...
  if command_entries.include? action_type["type"] 
      ...
      when "%reload%"
        ...
        container_namespace = generate_namespace(params["container_name"]) 
      when /%user:[\w]+%/
        user = action.split.first.gsub("%", '').split(':')[1]
        action = "su #{user} - -c \"#{action.split[1..-1].join(" ")}\""
      when /%run\[.*\]%/
        mappings = action.split.first.gsub(/[%\[\]]/, '').split(',')
        mappings.each do |mapping| 
          var, cmd = mappings.split(':') 
          stdout, stderr, status = Open3.capture3("podman exec #{params["container_name"]} #{cmd}")
          action = action.gsub(/%\|#{var}\|%/, stdout)
        end
